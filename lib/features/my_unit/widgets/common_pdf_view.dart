...
class PdfCommonViewPage extends StatefulWidget {
  final String pdfUrl;
  final String? title;
  final String? subTitle;
  final bool fromFilePicker;

  const PdfCommonViewPage({
    super.key,
  ...
  @override
  State<PdfCommonViewPage> createState() => _PdfCommonViewPageState();
}

class _PdfCommonViewPageState extends State<PdfCommonViewPage> {
  final PdfViewerController _pdfViewerController = PdfViewerController();
  bool get isNetworkUrl => widget.pdfUrl.startsWith('http://') || widget.pdfUrl.startsWith('https://');
  ...
  Future<String?> saveFile(BuildContext context, String filePath, String fileName) async {
    try {
      ...
      WorkplaceWidgets.successToast( "PDF Saved Successfully!\nPath: $newPath");
      return newPath;
    } catch (e) {
  ...
  void onUpload() {
    Navigator.pop(context, widget.pdfUrl);
  }
  ...
  Widget build(BuildContext context) {
    print('PdfCommonViewPage: Loading PDF from ${widget.pdfUrl}');
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: widget.fromFilePicker
          ...
          IconButton(
            ...
            onPressed: () => saveFile(context, widget.pdfUrl, path.basename(widget.pdfUrl)),
          ),
          const SizedBox(width: 8),
          IconButton(
            icon: const Icon(Icons.share, color: Colors.black, size: 24),
            onPressed: () => shareFile(context, widget.pdfUrl),
          ),
          const SizedBox(width: 8),
        ],
      ...
      body: widget.pdfUrl.isNotEmpty
        ...
        onDocumentLoadFailed: (details) {
          print('SfPdfViewer.network error: ${details.error}, ${details.description}');

          WorkplaceWidgets.errorSnackBar(context, 'Failed to load PDF: ${details.error}');
        },
      )
          : SfPdfViewer.file(
        File(widget.pdfUrl),
        controller: _pdfViewerController,
        onDocumentLoadFailed: (details) {
          print('SfPdfViewer.file error: ${details.error}, ${details.description}');
          WorkplaceWidgets.errorSnackBar(context, 'Failed to load PDF: ${details.error}');
        },
      )
          : const Center(
